...
const getBooksInYear= async function (req, res) {
    ...
    let booksYear= await BookModel.find({"year" : {$eq : years} }).select({bookName : 1, year : 1, _id : 0});
    res.send({msg : booksYear})
}
const getParticularBooks = async function (req, res) {
    let input = req.body;
  let booksinput= await BookModel.find(input);
    res.send({msg : booksinput});
}
const getXINRBook= async function (req, res) {
    let booksprice= await BookModel.find({"prices.indianPrice" : {$in : ["100 INR" , "200 INR" , "500 INR"]}}).select({bookName : 1,prices : 1, _id : 0});
    res.send({msg : booksprice})
}
const getRandomBooks = async function(req,res){
    let stockbook = await BookModel.find({stockAvailable : true, totalPages : {$gt : 500}}).select({bookName : 1, stockAvailable : 1, totalPages : 1, _id : 0});
    res.send({msg : stockbook});
}
